import { DirectionsRenderer, GoogleMap, Marker, OverlayView, Polyline, useJsApiLoader } from '@react-google-maps/api';
...
export default function GoogleMapView() {
  ...
  const generateMarkerComponent = async () => {
    const items = await Promise.all(markerItems.map(async (item) => {
      return (
        <div key={item.name}>
          <Marker position={item.location} 
            options={{
              icon: {
                url: (item.name==='mittaganahalli_DumpYard' ? DumpYard : (item.name==='Sampigehalli_GarbageVan') ? GarbageVan : DustBinPng),
